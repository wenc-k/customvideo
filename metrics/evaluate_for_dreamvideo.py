...
def extract_content(s):
    first = s.find('_')          # 第一个 _ 的位置
    last = s.rfind('_')          # 最后一个 _ 的位置
    if first == -1 or last == -1 or first == last:
        return ""                # 没有足够的 _，返回空字符串
    return s[first+1:last]       # 截取中间内容

# model path TODO.
dino_model_folder = '/home/wangwc/project/CogVideoX/finetune/metrics/dino-vits16'
...
all_result = []
eval_base_path = '/home/wangwc/project/CogVideoX/finetune/joint_custom' # 要测试的part TODO.
image_dataset_base_path = '/home/wangwc/project/CogVideoX/finetune/data/custom_image_dataset/one_part_train'
final_result_dir = os.path.join("/home/wangwc/project/CogVideoX/finetune", "dreamvideo_eval_rusult")
...
for dir_name in os.listdir(eval_base_path):
    ...
    json_path = os.path.join(cases_dir_path, 'eval.json')
    ...
    id_dataset_name = extract_content(dir_name)
    id_dataset_path = os.path.join(image_dataset_base_path, id_dataset_name)
    image_dataset_folder = os.path.join(id_dataset_path, "images")
    if os.path.isdir(image_dataset_folder):
        print("use image_dataset_folder: ", image_dataset_folder)
    else:
        exit("image_dataset_folder not exist: ")
...
output_func(final_result_dir, all_result)